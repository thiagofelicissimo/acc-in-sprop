...
| mkHasRenInterpTm : forall (iu : ZFSet -> ZFSet) (fu : interp_tm Δ l (ρ ⋅ t) iu) (vtu : ∀ δ ∈ iΔ, iu δ ≡ it (iρ δ)),
...
Inductive has_ren_interp_proj (Γ : ctx) (l : level)
  ...
  (Δ : ctx) (iΔ : ZFSet) (fΔ : interp_ctx Δ iΔ)
  (ρ : nat -> nat) (iρ : ZFSet -> ZFSet) (fρ : interp_ren Δ Γ ρ iρ) : Prop :=
| mkHasRenInterpProj : forall (iy : ZFSet -> ZFSet) (fy : nth_proj Δ l (ρ x) iy) (vxy : ∀ δ ∈ iΔ, iy δ ≡ ix (iρ δ)),
...
Proof.
  ...
  - intros. specialize (H Δ iΔ fΔ ρ iρ fρ). destruct H as [ iA' fA' vA' ].
    assert (interp_ctx (Δ,, (ty lA, ρ ⋅ A)) (ctxExt lA iΔ iA')) as fΔ'.
    { apply interp_cons_rel. exact fΔ. exact fA'. }
    assert (interp_ren (Δ,, (ty lA, ρ ⋅ A)) (Γ,, (ty lA, A)) (upRen_term_term ρ) (fun γ => ⟨ iρ γ ; iA γ ⟩)) as fρu.
    { apply interp_ren_cons. 
    econstructor.
    + cbn. apply interp_pi_rr. apply fA'.

specialize (H0 (Δ,, (ty lA, ρ ⋅ A)) _ _ (upRen_term_term ρ ⋅ B) _ _)
