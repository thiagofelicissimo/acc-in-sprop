...
| here_rel : forall Γ l A iΓ iA ix, interp_ctx Γ iΓ -> interp_tm Γ (Ax (ty l)) A iA
             -> ∀ γa ∈ ctxExt l iΓ iA, ix γa = setSndSigma l iΓ (fun γ => 𝕌el l (iA γ)) γa
             -> nth_proj (Γ ,, (ty l , A)) (ty l) 0 ix

| there_rel : forall Γ l lA A y iΓ iA iy ix, interp_ctx Γ iΓ -> interp_tm Γ (Ax (ty lA)) A iA -> nth_proj Γ l y iy
              -> ∀ γa ∈ ctxExt lA iΓ iA, ix γa = iy (setFstSigma lA iΓ (fun γ => 𝕌el lA (iA γ)) γa)
              -> nth_proj (Γ ,, (ty lA , A)) l (S y) ix

| here_irr : forall Γ A iΓ iA ix, interp_ctx Γ iΓ -> interp_tm Γ (ty 0) A iA
             -> ∀ γa ∈ ctxExt 0 iΓ (boxTy_HO iA), ix γa = setSndSigma 0 iΓ iA γa
             -> nth_proj (Γ ,, (prop , A)) prop 0 ix

| there_irr : forall Γ l A y iΓ iA iy ix, interp_ctx Γ iΓ -> interp_tm Γ (ty 0) A iA -> nth_proj Γ l y iy
              -> ∀ γa ∈ ctxExt 0 iΓ (boxTy_HO iA), ix γa = iy (setFstSigma 0 iΓ (fun γ => 𝕌el 0 (boxTy_HO iA γ)) γa)
              -> nth_proj (Γ ,, (prop , A)) l (S y) ix
...
| interp_type : forall Γ l iΓ it, interp_ctx Γ iΓ
                -> ∀ γ ∈ iΓ, it γ ≡ univTy_HO l γ
                -> interp_tm Γ (Ax (Ax (ty l))) (Sort (ty l)) it

| interp_prop : forall Γ iΓ it, interp_ctx Γ iΓ
                -> ∀ γ ∈ iΓ, it γ ≡ propTy_HO γ
                -> interp_tm Γ (ty 1) (Sort prop) it

| interp_pi_rr : forall Γ lA lB A B iA iB iΓ it, interp_ctx Γ iΓ
                 -> interp_tm Γ (Ax (ty lA)) A iA
                 -> interp_tm (Γ ,, (ty lA , A)) (Ax (ty lB)) B iB
                 -> ∀ γ ∈ iΓ, it γ ≡ piTy_HO lA lB iA iB γ
                 -> interp_tm Γ (Ax (Ru (ty lA) (ty lB))) (Pi (ty lA) (ty lB) A B) it

| interp_pi_ir : forall Γ lB A B iA iB iΓ it, interp_ctx Γ iΓ
                 -> interp_tm Γ (Ax prop) A iA
                 -> interp_tm (Γ ,, (prop , A)) (Ax (ty lB)) B iB
                 -> ∀ γ ∈ iΓ, it γ ≡ piTy_HO 0 lB (boxTy_HO iA) iB γ
                 -> interp_tm Γ (Ax (ty lB)) (Pi prop (ty lB) A B) it

| interp_pi_ri : forall Γ lA A B iA iB iΓ it, interp_ctx Γ iΓ
                 -> interp_tm Γ (Ax (ty lA)) A iA
                 -> interp_tm (Γ ,, (ty lA , A)) (Ax prop) B iB
                 -> ∀ γ ∈ iΓ, it γ ≡ forallTy_HO lA iA iB γ
                 -> interp_tm Γ (Ax prop) (Pi (ty lA) prop A B) it

| interp_pi_ii : forall Γ A B iA iB iΓ it, interp_ctx Γ iΓ
                 -> interp_tm Γ (Ax prop) A iA
                 -> interp_tm (Γ ,, (prop , A)) (Ax prop) B iB
                 -> ∀ γ ∈ iΓ, it γ ≡ forallTy_HO 0 (boxTy_HO iA) iB γ
                 -> interp_tm Γ (Ax prop) (Pi prop prop A B) it

| interp_lam_rr : forall Γ lA lB A B t iA it iΓ iv, interp_ctx Γ iΓ
                  -> interp_tm Γ (Ax (ty lA)) A iA
                  -> interp_tm (Γ ,, (ty lA , A)) (ty lB) t it
                  -> ∀ γ ∈ iΓ, iv γ ≡ lamTm_HO lA lB iA it γ
                  -> interp_tm Γ (Ru (ty lA) (ty lB)) (lam (ty lA) (ty lB) A B t) iv

| interp_lam_ir : forall Γ lB A B t iA it iΓ iu, interp_ctx Γ iΓ
                  -> interp_tm Γ (Ax prop) A iA
                  -> interp_tm (Γ ,, (prop , A)) (ty lB) t it
                  -> ∀ γ ∈ iΓ, iu γ ≡ lamTm_HO 0 lB (boxTy_HO iA) it γ
                  -> interp_tm Γ (ty lB) (lam prop (ty lB) A B t) iu

| interp_app_rr : forall Γ lA lB A B t u iA it iu iΓ iv, interp_ctx Γ iΓ
                  -> interp_tm Γ (Ax (ty lA)) A iA
                  -> interp_tm Γ (Ru (ty lA) (ty lB)) t it
                  -> interp_tm Γ (ty lA) u iu
                  -> ∀ γ ∈ iΓ, iv γ ≡ appTm_HO lA lB iA it iu γ
                  -> interp_tm Γ (ty lB) (app (ty lA) (ty lB) A B t u) iv

| interp_app_ir : forall Γ lB A B t u iA it iu iΓ iv, interp_ctx Γ iΓ
                  -> interp_tm Γ (Ax prop) A iA
                  -> interp_tm Γ (ty lB) t it
                  -> interp_tm Γ prop u iu
                  -> ∀ γ ∈ iΓ, iv γ ≡ appTm_HO 0 lB (boxTy_HO iA) it iu γ
                  -> interp_tm Γ (ty lB) (app prop (ty lB) A B t u) iv

| interp_nat : forall Γ iΓ it, interp_ctx Γ iΓ
               -> ∀ γ ∈ iΓ, it γ ≡ natTy_HO γ
               -> interp_tm Γ (ty 1) Nat it

| interp_zero : forall Γ iΓ it, interp_ctx Γ iΓ
                -> ∀ γ ∈ iΓ, it γ ≡ zeroTm_HO γ
                -> interp_tm Γ (ty 0) zero it

| interp_succ : forall Γ t it iΓ iu, interp_ctx Γ iΓ
                -> interp_tm Γ (ty 0) t it
                -> ∀ γ ∈ iΓ, iu γ ≡ sucTm_HO it γ
                -> interp_tm Γ (ty 0) (succ t) iu

| interp_natrec : forall Γ l P pz ps m iP ipz ips im iΓ it, interp_ctx Γ iΓ
                  -> interp_tm (Γ ,, (ty 0 , Nat)) (Ax (ty l)) P iP
                  -> interp_tm Γ (ty l) pz ipz
                  -> interp_tm (Γ ,, (ty 0 , Nat) ,, (ty l , P)) (ty l) ps ips
                  -> interp_tm Γ (ty 0) m im
                  -> ∀ γ ∈ iΓ, it γ ≡ natrecTm_HO l iP ipz ips im γ
                  -> interp_tm Γ (ty l) (rec (ty l) P pz ps m) it

| interp_acc : forall Γ i A R a iA iR ia iΓ it, interp_ctx Γ iΓ
               -> interp_tm Γ (Ax (ty i)) A iA
               -> interp_tm (Γ ,, (ty i, A) ,, (ty i, S ⋅ A)) (Ax prop) R iR
               -> interp_tm Γ (ty i) a ia
               -> ∀ γ ∈ iΓ, it γ ≡ accTy_HO iA iR ia γ
               -> interp_tm Γ (Ax prop) (Core.acc (ty i) A R a) it

| interp_accelim : forall Γ i l A R a q P p iA iR ia iP ip iΓ it, interp_ctx Γ iΓ
                   -> interp_tm Γ (Ax i) A iA
                   -> interp_tm (Γ ,, (i, A) ,, (i, S ⋅ A)) (Ax prop) R iR
                   -> interp_tm (Γ ,, (i, A)) (Ax (ty l)) P iP
                   -> interp_tm Γ (ty l) p ip
                   -> interp_tm Γ i a ia
                   -> ∀ γ ∈ iΓ, it γ ≡ accelimTm_HO l iA iR iP ip ia γ
                   -> interp_tm Γ (ty l) (accel i (ty l) A R P p a q) it

| interp_obseq : forall Γ l A a b iA ia ib iΓ it, interp_ctx Γ iΓ
                   -> interp_tm Γ (Ax (ty l)) A iA
                   -> interp_tm Γ (ty l) a ia
                   -> interp_tm Γ (ty l) b ib
                   -> ∀ γ ∈ iΓ, it γ ≡ eqTy_HO iA ia ib γ
                   -> interp_tm Γ (Ax prop) (obseq (ty l) A a b) it

| interp_cast : forall Γ l A B e a iA iB ia iΓ it, interp_ctx Γ iΓ
                -> interp_tm Γ (Ax (ty l)) A iA
                -> interp_tm Γ (Ax (ty l)) B iB
                -> interp_tm Γ (ty l) a ia
                -> ∀ γ ∈ iΓ, it γ ≡ castTm_HO iA iB ia γ
                -> interp_tm Γ (ty l) (cast (ty l) A B e a) it.
...
Inductive is_functional_tm (Γ : ctx) (l : level) (t : term) (it : ZFSet -> ZFSet) (ft : interp_tm Γ l t it) : Prop :=
| mkIsFunctionalTm : forall (iΓ : ZFSet) (fΓ : interp_ctx Γ iΓ) (it' : ZFSet -> ZFSet) (ft' : interp_tm Γ l t it'),
                            (∀ γ ∈ iΓ, it γ ≡ it' γ) -> is_functional_tm Γ l t it ft.

Inductive is_functional_proj (Γ : ctx) (l : level) (x : nat) (ix : ZFSet -> ZFSet) (fx : nth_proj Γ l x ix) : Prop :=
| mkIsFunctionalProj : forall (iΓ : ZFSet) (fΓ : interp_ctx Γ iΓ) (ix' : ZFSet -> ZFSet) (fx' : nth_proj Γ l x ix'),
                              (∀ γ ∈ iΓ, ix γ ≡ ix' γ) -> is_functional_proj Γ l x ix fx.
...
Definition is_ren_interp_ctx (Γ : ctx) (iΓ : ZFSet) (fΓ : interp_ctx Γ iΓ) : Prop :=
  True.

Definition is_ren_interp_tm (Γ : ctx) (l : level) (t : term) (it : ZFSet -> ZFSet) (ft : interp_tm Γ l t it) : Prop :=
  forall Δ ρ iρ, interp_ren Δ Γ ρ iρ -> interp_tm Δ l (ρ ⋅ t) (fun δ => it (iρ δ)).

Definition is_ren_interp_proj (Γ : ctx) (l : level) (x : nat) (ix : ZFSet -> ZFSet) (fx : nth_proj Γ l x ix) : Prop :=
  forall Δ ρ iρ, interp_ren Δ Γ ρ iρ -> nth_proj Δ l (ρ x) (fun δ => ix (iρ δ)).

Lemma is_ren_interp : (forall Γ l t it ft, is_ren_interp_tm Γ l t it ft)
                      /\ (forall Γ iΓ fΓ, is_ren_interp_ctx Γ iΓ fΓ)
                      /\ (forall Γ l x ix fx, is_ren_interp_proj Γ l x ix fx).
Proof.
  apply interp_mutind.
  - intros Γ l x ix fx IHx Δ ρ iρ fρ. cbn. apply interp_var. now apply (IHx Δ).
  - admit.
  - admit.
  - admit.
  - admit.
  ...
  - admit.
  - admit.
  - intros Γ l A iΓ iA fΓ IHΓ fA IHA Δ ρ iρ fρ. inversion fρ ; subst ; clear fρ.
