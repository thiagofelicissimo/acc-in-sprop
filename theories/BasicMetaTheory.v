...
Lemma ConvSubst_weak_two Γ Δ σ θ l A l' B :
  Γ ⊢s σ ≡ θ : Δ →
  ⊢ Γ ,, (l, A) ,, (l', B ) ->
  Γ ,, (l, A) ,, (l', B) ⊢s (σ >> ren_term (S >> S)) ≡ (θ >> ren_term (S >> S)) : Δ.
Proof.
  intros. induction H.
  1:econstructor.
  econstructor;eauto. 
...
Lemma ConvSubst_weak_three Γ Δ σ θ l A l' B l'' C :
  Γ ⊢s σ ≡ θ : Δ →
  ⊢ Γ ,, (l, A) ,, (l', B ) ,, (l'', C) ->
  Γ ,, (l, A) ,, (l', B) ,, (l'', C) 
    ⊢s (σ >> ren_term (S >> S >> S)) ≡ (θ >> ren_term (S >> S >> S)) : Δ.
Proof.
  intros. induction H.
...
Lemma pre_conv_in_ctx_subst Γ Γ' Δ σ :
  Γ ⊢s σ : Δ →
  ⊢ Γ' ->
  ⊢ Γ' ≡ Γ →
  Γ' ⊢s σ : Δ.
...
Lemma WellSubst_up_conv Γ Δ l A A' σ :
  Γ ⊢s σ : Δ →
  Γ ⊢< Ax l > A' ≡ A <[ σ ] : Sort l ->
  Γ ⊢< Ax l > A <[ σ ] : Sort l →
  Γ ⊢< Ax l > A' : Sort l →
...
Lemma WellSubst_up_two_conv Γ Δ σ l l' B A A' B' :
  Γ ⊢s σ : Δ →
  ⊢ Γ ,, (l, A') ,, (l', B') ->
  ⊢ Γ ,, (l, A <[ σ ]) ,, (l', B <[ up_term σ ]) ->
  ⊢ Γ ,, (l, A') ,, (l', B') ≡ Γ ,, (l, A <[ σ ]) ,, (l', B <[ up_term σ ]) ->
  Γ ,, (l, A') ,, (l', B') ⊢s up_term (up_term σ) : Δ ,, (l, A) ,, (l', B).
Proof.
  intros.
  eapply pre_conv_in_ctx_subst.
...
Proof.
  ...
  - assert (Δ,, (ty 0, Nat) ⊢< Ax l > P <[ up_term_term σ] : Sort l)
    ...
    assert (Δ,, (ty 0, Nat) ⊢< Ax l > P <[ up_term_term σ'] : Sort l) as h_
      by (eapply typing_conversion_subst in ht1; eauto using typing, ctx_typing, WellSubst_up).
    assert (Δ,, (ty 0, Nat) ⊢< Ax l > P <[ up_term_term σ] ≡ P <[ up_term_term σ'] : Sort l) as h__.
    ...
    + rasimpl; reflexivity.
  - assert (Δ ⊢< Ax (ty n) > A <[σ] : Sort (ty n)) as Aσ_Wt.
    { eapply typing_conversion_subst in ht1; eauto using typing, ctx_typing, WellSubst_up. }
    assert (Δ ⊢< Ax (ty n) > A <[σ'] : Sort (ty n)) as Aσ'_Wt.
    { eapply typing_conversion_subst in ht1; eauto using typing, ctx_typing, WellSubst_up. }
    assert (Δ ⊢< Ax (ty n) > A <[σ] ≡ A <[σ']: Sort (ty n)) as Aσ_conv_Aσ.
    { eapply typing_conversion_subst in ht1; eauto using typing, ctx_typing, WellSubst_up. }
    assert (⊢ (Δ,, (ty n, A <[ σ])),, (ty n, (S ⋅ A) <[ up_term σ])) as ΔAAσ.
    ...
    assert (⊢ (Δ,, (ty n, A <[ σ])),, (ty n, (S ⋅ A) <[ up_term σ]) ≡ 
  ...
  assert ((Δ,, (ty n, A <[ σ])),, (ty n, A <[ σ >> ren_term S]) ⊢< Ax prop > 
  ...
  assert (((Δ,, (ty n, A <[ σ])),, (ty n, A <[ σ >> ren_term S])),,
    ...
    ⊢s ((var 1) .: σ >> ren_term (S >> (S >> S))) : Γ,, (ty n, A)) as σ1.
   { econstructor.
  ...
  assert (((Δ,, (ty n, A <[ σ'])),, (ty n, A <[ σ' >> ren_term S])),,
    (prop, R <[ (var 1) .: ((var 0) .: σ' >> ren_term (S >> S))])
    ⊢s ((var 1) .: σ' >> ren_term (S >> (S >> S))) : Γ,, (ty n, A)) as σ'1.
   { econstructor.
    1:ssimpl; eapply WellSubst_weak_three; eauto using ctx_typing.
    ssimpl.
    econstructor; eauto using ctx_typing.
  ...
  assert (((Δ,, (ty n, A <[ σ])),, (ty n, A <[ σ >> ren_term S])),,
        (prop, R <[ (var 1) .: ((var 0) .: σ >> ren_term (S >> S))]) 
        ⊢< Ax l > P <[ (var 1) .: σ >> ren_term (S >> (S >> S))] : Sort l) as Pσ.
  ...
  assert (⊢ (Δ,, (ty n, A <[ σ])),, (Ru (ty n) l, B <[ up_term_term σ])) as ΔABσ.
  {  econstructor; eauto using ctx_typing.
...
- assert (Δ ⊢< Ax (ty n) > A <[σ] : Sort (ty n)) as Aσ_Wt.
  ...
  assert (((Δ,, (ty n, A <[ σ])),, (ty n, A <[ σ >> ren_term S])),,
        (prop, R <[ (var 1) .: ((var 0) .: σ >> ren_term (S >> S))]) 
        ⊢< Ax (ty m) > P <[ (var 1) .: σ >> ren_term (S >> (S >> S))] : Sort (ty m)) as Pσ.
  { eapply subst_ty; eauto using ctx_typing. }
  ...
  assert (((Δ,, (ty n, A <[ σ])),, (ty n, A <[ σ >> ren_term S])),,
        (prop, R <[ (var 1) .: ((var 0) .: σ >> ren_term (S >> S))]) 
        ⊢< Ax (ty m) > P <[ (var 1) .: σ >> ren_term (S >> (S >> S))]
  ...
  assert (⊢ (Δ,, (ty n, A <[ σ])),, (Ru (ty n) (ty m), B <[ up_term_term σ])) as ΔABσ.
  {  econstructor; eauto using ctx_typing.
     unfold B, R', P'. rasimpl.
     econstructor. 1:dependent destruction ΔAAσ; eauto.
      eapply meta_lvl.
...
Section AccCompValidity.
  ...
  Context (Γ : ctx) (n : nat) (A R : term)
    (AWt : Γ ⊢< Ax (ty n) > A : Sort (ty n))
    (RWt : Γ ,, (ty n, A) ,, (ty n, S ⋅ A) ⊢< Ax prop > R : Sort prop).
  Let R0 := (1 .: (0 .: (S >> S))) ⋅ R.

  Lemma R0Wt : Γ,, (ty n, A),, (ty n, S ⋅ A) ⊢< Ax prop > R0 : Sort prop.
  Proof.
    unfold R0.
  ...
  Context (l : level) (P : term) (PWt : Γ ,, (ty n, A) ⊢< Ax l > P : Sort l).
  Let P0 := (1 .: (S >> S >> S)) ⋅ P.
  Let B := Pi (ty n) l (S ⋅ A) (Pi prop l R0 P0).
  Let P00 := (1.: (S >> S)) ⋅ P.

  Lemma P0Wt : Γ,, (ty n, A),, (ty n, S ⋅ A),, (prop, R0) ⊢< Ax l > P0 : Sort l.
  Proof.
    unfold P0.
  ...
  Lemma BWt : Γ,, (ty n, A)⊢< Ax (Ru (ty n) l) > B : Sort (Ru (ty n) l).
  Proof.
    unfold B.
    econstructor.
  ...
  Lemma P00Wt : Γ ,, (ty n, A) ,, (Ru (ty n) l, B) ⊢< Ax l > P00 : Sort l.
  Proof.
    unfold P00.
    eapply type_ren; eauto.
  ...
  Lemma RwkWt : Γ ,, (ty n, A) ,, (prop, t2),, (ty n, Awk) ,, (ty n, S ⋅ Awk) ⊢< Ax prop > Rwk : Sort prop.
  Proof.
    unfold Rwk, Awk.
    eapply type_ren; eauto.
  ...
  Lemma PwkWt : Γ ,, (ty n, A) ,, (prop, t2),, (ty n, Awk) ⊢< Ax l > Pwk : Sort l.
  Proof.
    unfold Pwk, Awk.
    eapply type_ren; eauto.
  ...
  Lemma pwkWt : Γ ,, (ty n, A) ,, (prop, t2) ,, (ty n, Awk) ,, (Ru (ty n) l, (up_ren (S >> S)) ⋅ B) ⊢< l > pwk : (up_ren (up_ren (S >> S))) ⋅ P00.
  Proof.
  ...
  Lemma t0Wt : Γ ,, (ty n, A) ,, (prop, t2) ⊢< prop > t0 : acc (ty n) Awk Rwk (var 1).
  Proof.
    unfold t0.
  ...
  Lemma t1Wt : Γ ,, (ty n, A) ,, (prop, t2) ⊢< l > t1 : Pwk <[ (var 1) .. ].
  Proof.
    unfold t1.
  ...
  Lemma P00Wt2 : (Γ,, (ty n, A)),, (prop, t2) ⊢< Ax l > P00 : Sort l.
  Proof.
  ...
  Lemma t3Wt : Γ ,, (ty n, A) ⊢< l > t3 : Pi prop l t2 P00.
  Proof.
  ...
  Lemma t4Wt : Γ ,, (ty n, A) ⊢< Ax l > t4 : Sort l.
  Proof.
  ...
  Lemma t5Wt : Γ ⊢< Ru (ty n) l > t5 : Pi (ty n) l A t4.
  Proof.
...
Section AccCompValidityConv.
  ...
  Lemma P00_conv_P00' : Γ ,, (ty n, A) ,, (Ru (ty n) l, B) ⊢< Ax l > P00 ≡ P00' : Sort l.
  Proof.
    unfold P00.
    eapply conv_ren; eauto using validity_ty_ctx.
    econstructor.
  ...
  Lemma Γ_A_B_conv : ⊢ Γ ,, (ty n, A') ,, (Ru (ty n) l, B') ≡ Γ ,, (ty n, A) ,, (Ru (ty n) l, B).
  Proof.
    econstructor. 1:econstructor.
...
Proof.
  ...
  - assert (⊢ (Γ,, (ty n, A')),, (ty n, S ⋅ A')).
    ...
    assert (⊢ (Γ,, (ty n, A')),, (ty n, S ⋅ A') ≡ (Γ,, (ty n, A)),, (ty n, S ⋅ A)).
    { intuition eauto. econstructor. 1:econstructor.
      all:eauto using conv_sym, ctx_conv_refl, validity_ty_ctx.
      eapply conv_ren; eauto using WellRen_S, conv_sym.
      econstructor; eauto using validity_ty_ctx. } 
    assert (⊢ Γ,, (ty n, A)) by (econstructor; eauto using validity_ty_ctx).
    assert (Γ,, (ty n, A) ⊢s (S ⋅ a .: (var 0 .: S >> var)) : (Γ,, (ty n, A)),, (ty n, S ⋅ A)).
    ...
    assert (Γ,, (ty n, A) ⊢< Ax prop > RR : Sort prop).
    { intuition eauto. unfold RR. eapply subst_ty; eauto. } 
    assert (⊢ (Γ,, (ty n, A)),, (prop, RR)).
    { econstructor; eauto. }
    split.
    1:econstructor.
...
Lemma type_accinv' Γ n A R a p b r l T :
    Γ ⊢< prop > p : acc (ty n) A R a →
    Γ ⊢< ty n > b : A →
    Γ ⊢< prop > r : R <[a.:b..] →
    T = acc (ty n) A R b →
    l = prop →
    Γ ⊢< l > accinv (ty n) A R a p b r : T.
Proof.
...
Lemma conv_accel' :
    ...
    P0 = P <[a ..] ->
    Γ ⊢< l > accel (ty n) l A R P p a q ≡ accel (ty n) l A' R' P' p' a' q' : P0.
Proof.
  intros. subst.
